...
class TiktokSdkV2Plugin : FlutterPlugin, MethodCallHandler, ActivityAware,
  ...
  override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        ...
        when (call.method) {
            ...
            "login" -> {
                ...
                val startResult = authApi.authorize(request, authType)
                if (startResult) {
                    loginResult = result
                } else {
                    result.error("unknown error", "unknown error", null)
                }
            }
            ...
            "share" -> {
                ...
                val mediaUrls = call.argument<ArrayList<String>>("mediaUrls") ?: ArrayList()
                var isSharingImage = call.argument<Boolean>("isSharingImage")
                var greenScreenEnabled = call.argument<Boolean>("greenScreenEnabled")

                val shareRequest = ShareRequest(
                    clientKey = clientKey ?: "",
                    mediaContent = MediaContent(
                        if (isSharingImage == true) MediaType.IMAGE else MediaType.VIDEO,
                        mediaUrls
                    ),
                    shareFormat = if (greenScreenEnabled == true) {
                        Format.GREEN_SCREEN
                    } else {
                        Format.DEFAULT
                    },
                    packageName = activity.packageName ?: "",
                    resultActivityFullPath = "${activity.packageName ?: ""}.${activity::class.simpleName}",
                )
                val shareResult = shareApi.share(shareRequest)
                if (shareResult == true) {
                    result.success(null)
                } else {
                    result.error("unknown error", "unknown error", null)
    ...
    override fun onNewIntent(intent: Intent): Boolean {
        if (activity == null)
            return true
        val auth = AuthApi(activity = activity!!)
        auth.getAuthResponseFromIntent(intent, redirectUrl = redirectUrl)?.let {
